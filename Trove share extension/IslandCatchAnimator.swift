...
/// Pure-UIKit fly-to-island animation (SwiftUI does not render in _UIHostedWindow).
final class IslandCatchAnimator {
    ...
    private func runFlight(in container: UIView) {
        ...
        let end = CGPoint(
            ...
            y: Self.islandTopY + islandPill.bounds.height / 2
        )
        let control = CGPoint(
            x: (start.x + end.x) / 2 + 50,
            y: (start.y + end.y) / 2 - 80
        )
        ...
        UIView.animate(withDuration: 0.65, delay: 0, options: .curveEaseInOut) {
            ...
            self.islandPill.frame = CGRect(
                x: (container.bounds.width - 270) / 2,
                y: Self.islandTopY - 16,
                width: 270,
                height: 68
            )
    ...
    private func absorb() {
        ...
        UIView.animate(withDuration: 0.28, delay: 0, usingSpringWithDamping: 0.62, initialSpringVelocity: 0) {
            self.islandPill.frame = CGRect(
                x: (container.bounds.width - 280) / 2,
                y: Self.islandTopY - 17,
                width: 280,
                height: 70
            )
            self.islandGlow.alpha = 1
            ...
            UIView.animate(withDuration: 0.35, delay: 0.05, usingSpringWithDamping: 0.72, initialSpringVelocity: 0) {
                self.islandPill.frame = CGRect(
                    x: (container.bounds.width - 110) / 2,
                    y: Self.islandTopY,
                    width: 110,
                    height: 36
                )
